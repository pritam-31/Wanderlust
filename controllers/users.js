...
module.exports.profile = async (req, res, next) => {
    try {
        ...
        // Attach listing references to each review when possible
        const userReviews = [];
        for (let review of reviews) {
            const listing = await Listing.findOne({ reviews: review._id });
            userReviews.push({
        ...
        // Wishlist count used as a proxy for trips when a bookings model is not present
        const wishlistCount = await Wishlist.countDocuments({ user: user._id });

        const userStats = {
            totalListings: userListings.length,
            totalReviews: reviews.length,
            totalEarnings: 0, // no bookings model available
            tripsCount: wishlistCount
        };
...
module.exports.trips = async (req, res, next) => {
    try {
        // Bookings/trips are not modeled in this project yet — return empty arrays safely
        const trips = {
            upcoming: [],
            completed: []
        };

        // Wishlist items (saved listings)
        const wishlist = await Wishlist.find({ user: req.user._id }).populate('listing');

        res.render("users/trips.ejs", { trips, wishlist });
    } catch (err) {
        next(err);
    }
};